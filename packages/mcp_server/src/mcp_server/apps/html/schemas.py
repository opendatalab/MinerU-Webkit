...
class FileUrlItem(BaseModel):
    file_id: str | None = Field(None, description="文件ID（可选）")
    file_url: str = Field(..., description="文件链接URL")
    url: str | None = Field(None, description="网页URL（可选）")
    name: str | None = Field(None, description="文件名称（可选）")
    type: str | None = Field(None, description="文件类型（可选）")
...
class FileParseRequest(BaseModel):
    ...
    compress: bool = Field(False, description="是否压缩转换结果")
    category: str | None = Field(None, description="上传的文件类型（可选）")
