...
def is_likely_math(content):
    ...
    # 规则1: 包含数学运算符
    math_operators = r'[+\-*/^=<>]'
    if re.search(math_operators, content):
        return True

    # 规则2: 包含 LaTeX 数学命令
    latex_commands = r'\\[a-zA-Z]+|\{[^}]*\}'
    if re.search(latex_commands, content):
        return True

    # 规则3: 是单个变量（如 x, y, α, β）
    if re.match(r'^[a-zA-Zα-ωΑ-Ω_][a-zA-Z0-9α-ωΑ-Ω_]*$', content) and len(content) <= 3:
        return True

    # 规则4: 包含数学结构
    math_structures = r'\([^)]+\)|\[[^\]]+\]|\{[^}]+\}'
    if re.search(math_structures, content):
        return True

    # 规则5: 排除明显不是数学的情况
    # 5.1 包含空格且没有数学符号 -> 可能是自然语言
    if ' ' in content and not re.search(math_operators, content) and len(content.replace("$", "").strip()) > 1:
        return False

    # 5.2 看起来像货币金额
    if re.match(r'^\$\d+(?:\.\d+)?[kKmMbB]?\s+\b', content):
        return False

    # 5.3 包含常见英语单词
    english_words = ['the', 'and', 'for', 'with', 'that', 'this', 'have', 'from', 'to']
