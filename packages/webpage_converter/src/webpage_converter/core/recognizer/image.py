...
class ImageRecognizer(BaseHTMLElementRecognizer):
    ...
    def __parse_html_img(
        ...
        base_img = html_obj.xpath(
            '//*[starts-with(@xlink:href, "data:image/") and not(self::img)]',
    ...
    def __parse_img_attr(self, base_url: str, elem: HtmlElement, attributes: dict):
        ...
        src = self.__parse_img_text(elem_attributes)
        if src:
            if src.startswith("data:image"):
                ...
                attributes["text"] = src
                attributes["format"] = "url"
            else:
                attributes["text"] = self.__get_full_image_url(base_url, src)
        common_attributes = [
            "alt",
            "title",
            "width",
