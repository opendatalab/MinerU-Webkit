...
from ..utils.text_utils import normalize_text_segment


class HTMLFileFormatPostConverter(BaseFileFormatPostConverter):
    """一个从html文件中提取数据的提取器.
...
class ContentListStripSpacePostConverter(BaseFileFormatPostConverter):
    """对段落文本进行处理：
    1. 连续的多个空格转换成1个
    2. 连续的\t转换成1个
    3. 连续的\n转换成1个
...
class ContentListStaticsPostConverter(BaseFileFormatPostConverter):
